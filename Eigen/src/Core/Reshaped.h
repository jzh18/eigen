...
struct reshaped_evaluator<ArgType, Rows, Cols, Order, /* HasDirectAccess */ false>
  ...
  inline constexpr PacketScalar packet(Index rowId, Index colId) const
  {
    const RowCol row_col = index_remap(rowId, colId);
  ...
  template<int LoadMode>
  EIGEN_DEVICE_FUNC
  inline constexpr void writePacket(Index rowId, Index colId, const PacketScalar& val)
  {
    const RowCol row_col = index_remap(rowId, colId);
  ...
  template<int LoadMode>
  EIGEN_DEVICE_FUNC
  inline constexpr PacketScalar packet(Index index) const
  {
  ...
  template<int LoadMode>
  EIGEN_DEVICE_FUNC
  inline constexpr void writePacket(Index index, const PacketScalar& val)
  {
    const RowCol row_col = index_remap(RowsAtCompileTime == 1 ? 0 : index,
                                        RowsAtCompileTime == 1 ? index : 0);
