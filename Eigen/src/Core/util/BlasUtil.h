...
struct get_factor {
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr To run(const From& x) { return To(x); }
};

template <typename Scalar>
struct get_factor<Scalar, typename NumTraits<Scalar>::Real> {
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr typename NumTraits<Scalar>::Real run(const Scalar& x) {
...
class BlasVectorMapper {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr BlasVectorMapper(Scalar* data) : m_data(data) {}

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr Scalar operator()(Index i) const { return m_data[i]; }
  template <typename Packet, int AlignmentType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr Packet load(Index i) const {
    return ploadt<Packet, AlignmentType>(m_data + i);
  }

  template <typename Packet>
  EIGEN_DEVICE_FUNC constexpr bool aligned(Index i) const {
...
class BlasLinearMapper<Scalar, Index, AlignmentType> {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr BlasLinearMapper(Scalar* data, Index incr = 1) : m_data(data) {
    EIGEN_ONLY_USED_FOR_DEBUG(incr);
    eigen_assert(incr == 1);
  }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void prefetch(Index i) const { internal::prefetch(&operator()(i)); }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr Scalar& operator()(Index i) const { return m_data[i]; }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketType loadPacket(Index i) const {
    return ploadt<PacketType, AlignmentType>(m_data + i);
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketType loadPacketPartial(Index i, Index n,
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketType load(Index i) const {
    return ploadt<PacketType, AlignmentT>(m_data + i);
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void storePacket(Index i, const PacketType& p) const {
    pstoret<Scalar, PacketType, AlignmentType>(m_data + i, p);
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void storePacketPartial(Index i, const PacketType& p, Index n,
                                                                          Index offset = 0) const {
...
struct PacketBlockManagement {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void store(Scalar* to, const Index stride, Index i, Index j,
...
class blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, 1> {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr blas_data_mapper(Scalar* data, Index stride, Index incr = 1)
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void prefetch(Index i, Index j) const {
  ...
  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketType loadPacket(Index i, Index j) const {
    return ploadt<PacketType, AlignmentType>(&operator()(i, j));
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketType loadPacketPartial(Index i, Index j, Index n,
                                                                               Index offset = 0) const {
    return ploadt_partial<PacketType, AlignmentType>(&operator()(i, j), n, offset);
  }

  template <typename PacketT, int AlignmentT>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketT load(Index i, Index j) const {
    return ploadt<PacketT, AlignmentT>(&operator()(i, j));
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void storePacket(Index i, Index j, const PacketType& p) const {
    pstoret<Scalar, PacketType, AlignmentType>(&operator()(i, j), p);
  }

  template <typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void storePacketPartial(Index i, Index j, const PacketType& p,
                                                                          Index n, Index offset = 0) const {
    pstoret_partial<Scalar, PacketType, AlignmentType>(&operator()(i, j), p, n, offset);
  }

  template <typename SubPacket>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void scatterPacket(Index i, Index j, const SubPacket& p) const {
    pscatter<Scalar, SubPacket>(&operator()(i, j), p, m_stride);
  }

  template <typename SubPacket>
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr SubPacket gatherPacket(Index i, Index j) const {
  ...
  EIGEN_DEVICE_FUNC constexpr Index firstAligned(Index size) const {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void storePacketBlock(Index i, Index j,
...
class BlasLinearMapper {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr BlasLinearMapper(Scalar* data, Index incr)
      : m_data(data), m_incr(incr) {}

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void prefetch(int i) const { internal::prefetch(&operator()(i)); }
...
class blas_data_mapper {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr blas_data_mapper(Scalar* data, Index stride, Index incr)
  ...
  struct storePacketBlock_helper {
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr void store(
...
class const_blas_data_mapper : public blas_data_mapper<const Scalar, Index, StorageOrder> {
 public:
  ...
  EIGEN_ALWAYS_INLINE constexpr const_blas_data_mapper(const Scalar* data, Index stride)
      : blas_data_mapper<const Scalar, Index, StorageOrder>(data, stride) {}

  EIGEN_ALWAYS_INLINE constexpr SubMapper getSubMapper(Index i, Index j) const {
...
struct blas_traits {
  ...
  EIGEN_DEVICE_FUNC static inline EIGEN_DEVICE_FUNC constexpr ExtractType extract(const XprType& x) { return x; }
  EIGEN_DEVICE_FUNC static inline EIGEN_DEVICE_FUNC constexpr const Scalar extractScalarFactor(const XprType&) {
    return Scalar(1);
  }
};
...
struct blas_traits<CwiseUnaryOp<scalar_conjugate_op<Scalar>, NestedXpr> > : blas_traits<NestedXpr> {
  ...
  EIGEN_DEVICE_FUNC static inline constexpr ExtractType extract(const XprType& x) {
    return Base::extract(x.nestedExpression());
  }
  EIGEN_DEVICE_FUNC static inline constexpr Scalar extractScalarFactor(const XprType& x) {
...
struct blas_traits<
  ...
  typedef CwiseBinaryOp<scalar_product_op<Scalar>, const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain>, NestedXpr>
      XprType;
  typedef typename Base::ExtractType ExtractType;
  EIGEN_DEVICE_FUNC static inline EIGEN_DEVICE_FUNC constexpr ExtractType extract(const XprType& x) {
    return Base::extract(x.rhs());
  }
  EIGEN_DEVICE_FUNC static inline EIGEN_DEVICE_FUNC constexpr Scalar extractScalarFactor(const XprType& x) {
...
struct blas_traits<
  ...
  typedef CwiseBinaryOp<scalar_product_op<Scalar>, NestedXpr, const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain> >
      XprType;
  typedef typename Base::ExtractType ExtractType;
  EIGEN_DEVICE_FUNC static inline constexpr ExtractType extract(const XprType& x) { return Base::extract(x.lhs()); }
  EIGEN_DEVICE_FUNC static inline constexpr Scalar extractScalarFactor(const XprType& x) {
...
struct extract_data_selector {
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static constexpr const typename T::Scalar* run(const T& m) {
...
struct extract_data_selector<T, false> {
  EIGEN_DEVICE_FUNC static constexpr typename T::Scalar* run(const T&) { return 0; }
};

template <typename T>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr const typename T::Scalar* extract_data(const T& m) {
...
struct combine_scalar_factors_impl {
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static constexpr ResScalar run(const Lhs& lhs, const Rhs& rhs) {
    return blas_traits<Lhs>::extractScalarFactor(lhs) * blas_traits<Rhs>::extractScalarFactor(rhs);
  }
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static constexpr ResScalar run(const ResScalar& alpha, const Lhs& lhs,
                                                                       const Rhs& rhs) {
...
struct combine_scalar_factors_impl<bool, Lhs, Rhs> {
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static constexpr bool run(const Lhs& lhs, const Rhs& rhs) {
    return blas_traits<Lhs>::extractScalarFactor(lhs) && blas_traits<Rhs>::extractScalarFactor(rhs);
  }
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static constexpr bool run(const bool& alpha, const Lhs& lhs, const Rhs& rhs) {
    return alpha && blas_traits<Lhs>::extractScalarFactor(lhs) && blas_traits<Rhs>::extractScalarFactor(rhs);
  }
};

template <typename ResScalar, typename Lhs, typename Rhs>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr ResScalar combine_scalar_factors(const ResScalar& alpha, const Lhs& lhs,
                                                                                 const Rhs& rhs) {
  return combine_scalar_factors_impl<ResScalar, Lhs, Rhs>::run(alpha, lhs, rhs);
}
template <typename ResScalar, typename Lhs, typename Rhs>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr ResScalar combine_scalar_factors(const Lhs& lhs, const Rhs& rhs) {
