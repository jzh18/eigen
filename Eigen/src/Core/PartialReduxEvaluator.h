...
struct evaluator<PartialReduxExpr<ArgType, MemberOp, Direction> >
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketType packet(Index i, Index j) const {
    return packet<LoadMode, PacketType>(Direction == Vertical ? j : i);
  }

  template <int LoadMode, typename PacketType>
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC constexpr PacketType packet(Index idx) const {
