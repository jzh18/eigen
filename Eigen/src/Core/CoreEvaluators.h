...
struct evaluator<PlainObjectBase<Derived>> : evaluator_base<Derived> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketType packet(Index row, Index col) const {
    if (IsRowMajor)
  ...
  template <int LoadMode, typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketType packet(Index index) const {
    return ploadt<PacketType, LoadMode>(m_d.data + index);
  }

  template <int StoreMode, typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr void writePacket(Index row, Index col, const PacketType& x) {
    if (IsRowMajor)
  ...
  template <int StoreMode, typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr void writePacket(Index index, const PacketType& x) {
...
struct evaluator<CwiseNullaryOp<NullaryOp, PlainObjectType>>
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketType packet(IndexType row, IndexType col) const {
    return m_wrapper.template packetOp<PacketType>(m_functor, row, col);
  }

  template <int LoadMode, typename PacketType, typename IndexType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketType packet(IndexType index) const {
...
struct unary_evaluator<CwiseUnaryOp<core_cast_op<SrcType, DstType>, ArgType>, IndexBased> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr DstPacketType packet(Index row, Index col) const {
  ...
  template <int LoadMode, typename DstPacketType, AltSrcScalarOp<DstPacketType> = true>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr DstPacketType packet(Index index) const {
    constexpr int DstPacketSize = unpacket_traits<DstPacketType>::size;
    constexpr int SrcBytesIncrement = DstPacketSize * sizeof(SrcType);
    constexpr int SrcLoadMode = plain_enum_min(SrcBytesIncrement, LoadMode);
    SrcPacketType src;
