...
struct TensorEvaluator<const TensorBroadcastingOp<Broadcast, ArgType>, Device> {
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr PacketReturnType packet(Index index) const {
    if (internal::is_input_scalar<internal::remove_all_t<InputDimensions>>::value) {
  ...
  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketReturnType packetOneByNByOne(Index index) const {
  ...
  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketReturnType packetOneByN(Index index) const {
  ...
  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketReturnType packetNByOne(Index index) const {
    // Consider the flattened tensor [v0, ..., vN],
