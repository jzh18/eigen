...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC constexpr PacketReturnType packet(const Index index) const {
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice> {
  ...
  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr PacketReturnType packet(const Index index) const {
    eigen_assert(m_buf);
    eigen_assert(index < m_dimensions.TotalSize());
